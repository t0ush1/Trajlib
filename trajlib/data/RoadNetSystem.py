...
class RoadNetSystem:
    ...
    def _get_roadnetwork(self) -> Tuple[InMemMap, gpd.GeoDataFrame, gpd.GeoDataFrame]:
        ...
        cache_filepath = os.path.join(
            self.cache_dir, f"cache_{self.network_type}_{bounds_name}.graphml"
        )
        ...
        edges["_osmid_hashable"] = edges["osmid"].apply(
    ...
    def get_road_osmids_for_points(self, coordinates: List[Tuple[float, float]], timestamps, filter_method="cut") -> Tuple[List[str], List[Tuple[int, int]]]:
        """
    ...
    def _match_gps_path(self, lon_lat_list: List[Tuple[float, float]], visualization: bool = False) -> Tuple[List[Tuple[int, int]], DistanceMatcher]:
        """单个GPS路径匹配"""
